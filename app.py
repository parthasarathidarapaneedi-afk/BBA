...
if not is_admin:
    ...
    m = match
    if tab == "score":
        if m.first_court_popup_point:
            st.markdown(f"<div class='court-alert'>🔄 Court change at {m.first_court_popup_point} — shown only for the team that reached it first</div>", unsafe_allow_html=True)
        if m.over:
...
if not match and tab == "score":
    ...
    if not m:
        ...
        if m.first_court_popup_point:
            st.markdown(f"<div class='court-alert'>🔄 Court change popup shown only once for first team reaching {m.first_court_popup_point}</div>", unsafe_allow_html=True)
        if m.over:
